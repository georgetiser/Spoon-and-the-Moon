...
REM these were not tested
REM ls *.png | grep "-a" | cut -c46-99 >> genie-media.txt
REM ls *.mp3 | grep "-a" | cut -c46-99 >> genie-media.txt

ls *.lua | grep "page_" | cut -c46-99 >> genie-lua.txt

FOR /F "usebackq delims=" %%G IN (genie-media.txt) DO (
	REM echo %%G
	grep -l %%G *.lua | sed -n '1p' > genietemp.txt
	FOR /F "usebackq delims=" %%H IN (genietemp.txt) DO (
			echo %%G appears in %%H
			echo %%G appears in %%H >> genieresults.txt
			echo %%G >> genieneeds.txt
		)
	)

echo Done

REM echo x > geniefilelist.txt
REM echo x > genietofindlist.txt
REM echo x > genieresults.txt
REM FOR /F "usebackq delims=" %%G IN (genieneeds.txt) DO (cp -n "E:/Projects/Jobs/SATM/assetset/audioset/%%G" "E:/Projects/Jobs/SATM/src/book3/audio/%%G")
REM cat genieresults.txt
